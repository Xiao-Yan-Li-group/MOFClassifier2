...
def main(CIF_FOLDER, MAX_WORKERS, OUTPUT_DIR):
...
if __name__ == "__main__":
    CIF_FOLDER = "../../dataset/cifs"    ############################################## the folder including all CIFs
    MAX_WORKERS = 64          ############################################## number of cores (parallel)
    OUTPUT_DIR  = "MOSAEC" ############################################## output folder (meta data, JSON type)
    ...
    for file in tqdm(glob.glob(OUTPUT_DIR+"/*json")):
    ...
    data = pd.DataFrame(all_data, columns=["id", "impossible", "unknown",
                                        ...
                                        ).to_csv("MOSAEC.csv", index=False) ############################################## result (csv type)