...
def main():
    ...
    cif_folder = "../../dataset/cifs"
    all_cif_paths = glob.glob(os.path.join(cif_folder, "*cif"))[:]

    output_csv = "../../dataset/5checkers/MOFChecker.csv"

    if os.path.exists(output_csv):
        df = pd.read_csv(output_csv)
        processed_names = set(df['name'].tolist())
        cif_paths = [
            path for path in all_cif_paths 
            if Path(path).stem not in processed_names
        ]
    else:
        header = [
            "name",
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor, \
        ...
        future_to_cif = {executor.submit(process_cif, path): path for path in cif_paths}
